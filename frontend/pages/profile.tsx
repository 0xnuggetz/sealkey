...
function Collection() {
  ...
  if (address === "TCAPAECoeJL2wxBVQ1u2De7mMKAoo85ZbX") {
    return <SampleCollection address={address} />;
  }
  ...
  function SampleCollection({ address }) {
    // const collection = collections[address as string];
    // const provider = useProvider();
    const [tokens, setTokens] = useState([]);

    const fetchAllTokenURIs = useCallback(async () => {
      try {
        const secretTokenContract = await window.tronWeb
          .contract()
          .at(SECRET_TOKEN_ADDRESS);

        const lastTokenId = await secretTokenContract.getLastTokenId().call();
        const fetchedMetadata = [];

        for (let tokenId = 1; tokenId <= lastTokenId.toNumber(); tokenId++) {
          const tokenURI = await secretTokenContract
          ...
          if (tokenURI) {
            ...
            result.id = tokenId;
            fetchedMetadata.push(result);
          }
        }

        setTokens(fetchedMetadata);
      } catch (e) {
        console.error(e);
      }
    }, []);
      ...
      <HStack className={styles.main}>
        <VStack className={styles.detailContainer}>
          <VStack className={styles.detailContentContainer}>
            ...
            <Text className={styles.owner}>
              {`by ${abridgeAddress(address)}`}
            </Text>
            <Text className={styles.description}>{"hello"}</Text>
            <HStack gap={1}>
              <ChakraLink
        ...
        <VStack className={styles.gridContainer}>
          <SimpleGrid columns={4} w="100%" gap={3}>
            {[...tokens].map(({ image, name, owner, id }, idx) => (
